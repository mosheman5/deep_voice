function combine_plots_diff(h,varargin);
% calculate and plot the difference between the first to the second graph
...
data1=data(1).y;
data2 =data(2).y;
%data2 = interp1(data(2).x,data(2).y, data(1).x);
 datamaxy(1:length(data1)) = data1(1:end)-
 data2(1:end);
 datamaxy(datamaxy<0)=0;
